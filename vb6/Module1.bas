...
Function symbolToString(s As Variant, Optional doRaise As Boolean = True) As String
    ...
    End If
    If doRaise Then
        Err.Raise 1009, "Lisp", "symbol expected"
    End If
End Function
...
End Function
Function newHostFunction(host As Object, method As String, Optional listParam As Boolean = False, Optional calltype As VbCallType = VbMethod) As LInternalFunction
    Dim c As New LInternalFunction
...
Function IsMacroCall(ByRef o As Variant, ByRef env As LHashmap, ByRef macroFunction As LFunction) As Boolean
    If IsNonemptyList(o, False) Then
        ...
        Dim name As String: name = symbolToString(appl.item(0))
        If name <> "" Then
...
Function IsListOfLength(ByRef o As Variant, length As Integer) As Boolean
...
tco:
Dim f As Variant, i  As Integer, fun As LFunction
